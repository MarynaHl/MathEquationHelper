// package com.example.equations.database;

// import java.sql.Connection;
// import java.sql.PreparedStatement;
// import java.sql.ResultSet;
// import java.sql.SQLException;
// import java.util.ArrayList;
// import java.util.List;
// import java.util.logging.Level;
// import java.util.logging.Logger;

// public class EquationDAO {

//     private EquationDAO() {
//     }

//     public static void insertEquationAndRoots(String equation, String roots)
//         throws SQLException {
//         String insertQuery =
//             "INSERT INTO equations (equation, roots) VALUES (?, ?) ";
//         try (Connection connection = DatabaseManager.getConnection();
//             PreparedStatement preparedStatement = connection.prepareStatement(
//                 insertQuery)) {
//             preparedStatement.setString(1, equation);
//             preparedStatement.setString(2, roots);
//             preparedStatement.executeUpdate();
//         } catch (SQLException e) {
//             handleSQLException(e);
//         }
//     }

//     public static List<String> findEquationsByRoot(double root, double DELTA) {
//         List<String> equations = new ArrayList<>();
//         String findQuery = "SELECT equation, roots FROM equations";
//         try (Connection connection = DatabaseManager.getConnection();
//             PreparedStatement preparedStatement = connection.prepareStatement(
//                 findQuery);
//             ResultSet resultSet = preparedStatement.executeQuery()) {
//             while (resultSet.next()) {
//                 String rootsString = resultSet.getString("roots");
//                 String[] rootsArray = rootsString.split(",\\s*");

//                 for (String rootItem : rootsArray) {
//                     double storedRoot = Double.parseDouble(rootItem.trim());
//                     if (Math.abs(storedRoot - root) < DELTA) {
//                         String equation = resultSet.getString("equation");
//                         equations.add(equation);
//                         break;
//                     }
//                 }
//             }
//         } catch (SQLException e) {
//             handleSQLException(e);
//         }
//         return equations;
//     }

//     public static boolean equationExists(String equation) {
//         String checkQuery = "SELECT COUNT(*) FROM equations WHERE equation = ?";
//         try (Connection connection = DatabaseManager.getConnection();
//             PreparedStatement preparedStatement = connection.prepareStatement(
//                 checkQuery)) {
//             preparedStatement.setString(1, equation);
//             try (ResultSet resultSet = preparedStatement.executeQuery()) {
//                 if (resultSet.next()) {
//                     int count = resultSet.getInt(1);
//                     return count > 0;
//                 }
//             }
//         } catch (SQLException e) {
//             handleSQLException(e);
//         }
//         return false;
//     }

//     private static void handleSQLException(SQLException e) {
//         Logger.getLogger(EquationDAO.class.getName()).log(Level.SEVERE, null, e);
//     }
// }

// /**
//  * Клас EquationDAO відповідає за взаємодію з базою даних для зберігання та
//  * витягування інформації про рівняння.
//  * У класі реалізовані методи для вставки нового рівняння та його коренів в базу
//  * даних,
//  * пошуку рівнянь за заданим коренем та перевірки наявності рівняння в базі
//  * даних.
//  * 
//  * Метод insertEquationAndRoots використовує PreparedStatement для безпечного
//  * вставлення
//  * рівняння та його коренів у таблицю "equations" бази даних.
//  * 
//  * Метод findEquationsByRoot використовує PreparedStatement та ResultSet для
//  * витягування
//  * рівнянь з бази даних, які мають корені, близькі до заданого значення root з
//  * урахуванням DELTA.
//  * 
//  * Метод equationExists перевіряє наявність рівняння в базі даних за допомогою
//  * PreparedStatement
//  * та підраховує кількість відповідних рядків.
//  * 
//  * У випадку виникнення SQLException, викликається приватний метод
//  * handleSQLException,
//  * який логує інформацію про помилку.
//  */
